...
import { Fragment, TextareaHTMLAttributes, useState } from "react";
...
export async function getStaticProps({ params }: { params: { book_id: string, bab_id: string, hadith_id: string } }) {
...
export default function Home(props : { book: BabItem, hadith_id: number }) {
    ...
    let session : NeuralSpaceSession | null;

    async function toggleRecord() {
        
        if (!isRecording) {
            navigator.mediaDevices.getUserMedia(constraints).then(async (stream) => {
                session = await NeuralSpace.createSession("sk_2756af2dda30d9ed17f0062b174e40168d4c26dcc54fb89a201ecf7de0f2814c", 600)
                session?.socket.addEventListener('message', (ev) => {
                    try {
                ...
                session?.socket.addEventListener('open', (ev) => {
                    console.log("WebSocket connected")
                })
                session?.socket.addEventListener('error', (ev) => {
                    console.log("WebSocket Error")
                })
                context = new AudioContext()
                source = context.createMediaStreamSource(stream)
                let script = context.createScriptProcessor(4096, 1, 1)
                script.addEventListener('audioprocess', async (ev) => {
                    ...
                    if (session?.socket.readyState == WebSocket.OPEN)
                        session?.socket.send(int16Data.buffer);
                })
    ...
    return (
        ...
        <div className="flex flex-col items-center">
            <div className={`w-3/4 mt-10 text-xl p-5 ${Cairo.className}`}>
                ...
                <textarea name="sharh" id="sharh" className="my-5 border-2 border-gray-200 rounded w-full h-80 bg-gray-100" defaultValue={displayText}>
                </textarea>
